...
#' (\link{http://www.github.com/KopfLab/dpos/inst/gui/}
...
run_gui <- function(base_dir = ".", ...) {
  ...
  app_dir <- system.file("gui", package = "dpos")
  if (app_dir == "")
    stop("Could not find GUI directory. Try re-installing `dpos`.", call. = FALSE)

  dpos:::run_gui_dev(base_dir = base_dir, app_dir = app_dir, ...)
}
...
run_gui_dev <- function(base_dir = ".", app_dir = file.path("inst", "gui"), ...) {
  ...
  if (!file.exists(app_dir))
    stop("App directory does not exist, something might be wrong with the `dpos` installation: ", app_dir)
