...
server <- shinyServer(function(input, output, session) {
  ...
  upload_dir <- file.path(tempdir()) # this is NOT user specific but R session specific (so maybe user specific on an R server?)
  ...
  tuning_files <- callModule(fileSelector, "tuning_files_local",
                            root = data_dir, root_name = "Data", size = 10)



  # look into reactiveFileReader !!!
